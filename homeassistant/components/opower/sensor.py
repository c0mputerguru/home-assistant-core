...
async def async_setup_entry(
    ...
    for forecast in forecasts:
        device_id = f"{coordinator.api.utility.subdomain()}_{forecast.account.utility_account_id}_{forecast.account.service_point_id}"
        device = DeviceInfo(
            identifiers={(DOMAIN, device_id)},
            name=f"{forecast.account.meter_type.name} account {forecast.account.utility_account_id} service point {forecast.account.service_point_id}",
            manufacturer="Opower",
    ...
    def native_value(self) -> StateType:
        ...
        if self.coordinator.data is not None:
            return self.entity_description.value_fn(
                self.coordinator.data[self.uuid]
            )
        return None
